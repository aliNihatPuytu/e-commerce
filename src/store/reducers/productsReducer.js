...
export default function productReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case PRODUCT_SET_FETCH_STATE:
      return { ...state, fetchState: action.payload || "NOT_FETCHED" };
    ...
    case PRODUCT_SET_ACTIVE_PRODUCT:
      return { ...state, activeProduct: action.payload ?? null };

    case PRODUCT_SET_ACTIVE_FETCH_STATE:
      return { ...state, activeFetchState: action.payload || "NOT_FETCHED" };

    case PRODUCT_SET_LIKES:
      return {
        ...state,
        likedIds: Array.isArray(action.payload) ? action.payload.map(String) : [],
      };

    case PRODUCT_TOGGLE_LIKE: {
      const id = String(action.payload);
      const exists = state.likedIds.includes(id);
      return {
        ...state,
        likedIds: exists ? state.likedIds.filter((x) => x !== id) : [...state.likedIds, id],
      };
    }
